/*
 * Source guard for the event-driven zz9k.library multi-port smoke test.
 *
...
int main(int argc, char **argv)
{
  ...
  if (argc != 2) {
    printf("usage: %s <zz9k-libeventports.c>\n", argv[0]);
    return 2;
  }
  ...
  ok &= expect_contains(source, "ZZ9K_EVENT_PORTS");
  ...
  ok &= expect_contains(source, "completion_index");
  ok &= expect_contains(source, "event ports ok");
