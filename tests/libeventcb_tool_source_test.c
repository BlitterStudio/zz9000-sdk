/*
 * Source guard for the event-driven zz9k.library callback smoke test.
 *
...
int main(int argc, char **argv)
{
  ...
  if (argc != 2) {
    printf("usage: %s <zz9k-libeventcb.c>\n", argv[0]);
    return 2;
  }
  ...
  ok &= expect_contains(source, "ZZ9KCallAsync");
  ok &= expect_contains(source, "AllocSignal");
  ok &= expect_contains(source, "Signal(state->task");
  ...
  ok &= expect_contains(source, "event callback ok");
  ...
  ok &= expect_not_contains(source, "ZZ9KCallAsyncMsg");
