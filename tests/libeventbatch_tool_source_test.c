/*
 * Source guard for the event-driven zz9k.library batch message-port smoke test.
 *
...
int main(int argc, char **argv)
{
  ...
  if (argc != 2) {
    printf("usage: %s <zz9k-libeventbatch.c>\n", argv[0]);
    return 2;
  }
  ...
  ok &= expect_contains(source, "event batch ok");
  ...
  ok &= expect_not_contains(source, "ZZ9KWaitAsyncBatch(");
