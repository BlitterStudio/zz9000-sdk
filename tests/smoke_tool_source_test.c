/*
 * Source guard for zz9k-smoke graphics format choices.
 *
...
int main(int argc, char **argv)
{
  ...
  if (argc != 2) {
    printf("usage: %s <tools/zz9k-smoke.c>\n", argv[0]);
    return 2;
  }
  ...
  ok = 1;
  ok &= expect_contains(source, "fill_bgra_gradient");
  ...
  ok &= expect_not_contains(source, "fill_argb_gradient");
