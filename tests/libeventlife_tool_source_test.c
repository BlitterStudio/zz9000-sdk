/*
 * Source guard for the event-driven zz9k.library lifecycle smoke test.
 *
...
int main(int argc, char **argv)
{
  ...
  if (argc != 2) {
    printf("usage: %s <zz9k-libeventlife.c>\n", argv[0]);
    return 2;
  }
  ...
  ok &= expect_contains(source, "ZZ9K_LIFE_ROUNDS");
  ok &= expect_contains(source, "OpenLibrary");
  ok &= expect_contains(source, "CloseLibrary");
  ...
  ok &= expect_contains(source, "event lifecycle ok");
