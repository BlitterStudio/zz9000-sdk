/*
 * Source guard for the SDK completion IRQ diagnostic tool.
 *
...
int main(int argc, char **argv)
{
  ...
  if (argc != 2) {
    printf("usage: %s <tools/zz9k-irqtest.c>\n", argv[0]);
    return 2;
  }
  ...
  ok = 1;
  ok &= expect_contains(source, "zz9k_irqtest_install_irq");
  ...
  ok &= expect_contains(source, "zz9k_completion_irq_enable(ctx, 1)");
  ok &= expect_contains(source, "zz9k_completion_irq_enable(ctx, 0)");
  ...
  ok &= expect_contains(source, "zz9k_request_ping");
  ok &= expect_contains(source, "zz9k_submit");
  ok &= expect_contains(source, "zz9k_poll");
  ok &= expect_contains(source, "irqtest ok");
