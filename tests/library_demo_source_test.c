/*
 * Source guard for the basic public zz9k.library demo.
 *
...
int main(int argc, char **argv)
{
  ...
  if (argc != 2) {
    printf("usage: %s <examples/amiga-library/zz9k-library-demo.c>\n",
           argv[0]);
    return 2;
  }
  ...
  ok &= expect_contains(source, "#include \"zz9k/sdk.h\"");
  ...
  ok &= expect_contains(source, "zz9k_event_timer_open");
  ok &= expect_contains(source, "zz9k_event_wait_msg");
  ok &= expect_contains(source, "zz9k_event_wait_async_port");
  ok &= expect_contains(source, "async event ping: ok");
  ok &= expect_contains(source, "async event batch ping: ok");
  ...
  ok &= expect_not_contains(source, "DEMO_MAX_POLLS");
